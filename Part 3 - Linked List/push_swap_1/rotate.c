...
/*   Updated: 2026/03/25 18:49:42 by dganapat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
										...
										// everthing up by one. 
{
	...
	*stack = (*stack)->next; // Assigning the head node as the second node in the stack.
	(*stack)->previous = NULL; // Assigning the new head's previous pointer to NULL. (detaching it from the previous top)
	last_node->next->previous = last_node; // Saying the old head (1) which is now the new last node's(1) previous pointer 
