...
/*   Updated: 2026/03/25 18:59:19 by dganapat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void rev_rotate(t_stack_node **stack) // A function taht moes the last to first and moves everthing downwards on the stack.
{
	...
	if (!stack || !(*stack)->next) // To check if the stack is empty or if there is only one node.
		return ;
	last_n = find_last(*stack); // To find the last node and set it to last_n.
	last_n->previous->next = NULL; // Assigning th esecond last's node's next pointer to point ot NULL.
	last_n->next = *stack; // Getting the last's next to point to the current head node. 
	last_n->previous = NULL; // assigning the last's pervious to point to null.
	*stack = last_n; // assigning the head node to the last node. 
	last_n->next->previous = last_n; // gettting number 1 or the second node's previous to point to the new head node.
}
