...
/*   Updated: 2026/03/25 18:48:44 by dganapat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void push(t_stack_node **dst, t_stack_node **src) // A function that pushes the top of one node to another
{
	...
	*src = (*src)->next; // Assigning the new source to be the next node on the list as it will become the new Head node.
	if (*src) // Checks if the new top node exists.
		(*src)->previous = NULL; // Assigning the prev pointer of the new Head node to null, as it should be. 
	push_node->previous = NULL; // Detaching the push node from the stack. (Check this should be next instead of previous)
	...
	else
	{
		...
		push_node->next->previous = push_node; // Assiggning the current head node previous pointer pointing to the pushed node (New Head).
