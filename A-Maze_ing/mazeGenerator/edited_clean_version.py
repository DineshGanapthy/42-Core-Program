...
def main() -> None:
    try:
        # 1. Read and validate the configurations from file
        reader = ConfigReader()
        ...
        # 2. Feed the validated configuration directly into your MazeGenerator
        generator = MazeGenerator(
            ...
            seed=None # Dynamic random maze every run
        )
        ...
        # Re-using our visual printer helper from before
        print_maze_with_path(generated_maze, generator.path)

        # 3. Export to file if specified in config
        if config.output_file:
            # You can call your exporter here, or write a quick save file routine
            print(f"\nMaze processing finished. Saving to '{config.output_file}'...")
            with open(config.output_file, "w") as f:
                for y in range(generated_maze.grid_height):
                    row = "".join(["██" if cell == 1 else "▒▒" if cell == 2 else "  " for cell in generated_maze.grid[y]])
                    f.write(row + "\n")
...
#         if config.output_file:
#             maze.print_maze_with_path(config.output_file)
...
#         print(e)
    ...
    #     perfect=True,     # Breaks walls if False
    #     seed=None
    # )
